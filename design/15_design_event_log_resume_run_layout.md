...
**Which process the end of a helper names.** A pid is not evidence of the process holding it. An embedding host may reap this process's children from its own `SIGCHLD` handler with a wildcard wait; a helper collected there leaves its number free for the kernel to hand to another of that host's forks, and no observation the parent can make tells the two apart. So where the platform offers a name for a process that a reused number cannot impersonate, the launch takes one in the parent the instant `fork` returns and ends the helper through that name rather than through the number: on Linux a pid file descriptor, signalled with `pidfd_send_signal` and collected with `waitid` on the descriptor, neither of which can reach a process the descriptor does not name; and where the helper was collected before any name could be taken, the end reaches nothing at all rather than falling back to the number. Where no such name can be had — macOS, any supported Unix whose kernel lacks the facility, and a syscall policy that will not let this process use one of the calls the name is used through — the end stays a `kill` and a `waitpid` on the number, and **ending a helper there is best effort**: against a host that reaps this process's children while a helper is starting or ending, it may reach a process that is not the helper. **Whether this host offers those calls is established by making them once in a forked child**, not by making them here and reading what came back. A policy may *kill* the caller rather than answer it — that is the default disposition for a disallowed call under systemd's `SystemCallFilter=` — and a process ended by a system call takes no fallback, so a policy that kills is the same platform row as one that refuses and what dies is the probe. A policy may also *write* the errno the kernel would have used about a helper, `ESRCH` for a signal and `ECHILD` for a wait; those two are read as the kernel's own only where the child's probe found this host giving them truthfully for a process whose fate it already knew, and otherwise they establish nothing and the end falls back to the number. A refusal that selects on the teardown's own arguments is answered where that call is made, for the same reason: a launch's arguments are not a teardown's. **What never falls back is a name this platform offers and this launch could not take**: a descriptor the process had run out of says nothing about the facility, so the launch fails and the helper ends on its closed command pipe rather than under a signal aimed at a number — and it is collected by number there, because a wait reaches none but this process's own children and an uncollected child's number is not re-issued, where a `kill` aimed at that number has no such guarantee. Upstroke asks no obligation of an embedding host here; it states which platforms it can answer for and which it cannot. Beyond that name and the pipe it was already reading, the parent asks the kernel nothing about the helper, and the message says only what the pipe carried and what the signal and the wait answered.
