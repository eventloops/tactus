...
**When a Unix helper does not start.** The cleanup reaper and the job-control guard are forked before any agent exists, and each acknowledges its own startup within a fixed budget. A launch that does not see that acknowledgement fails, ends the helper with one `SIGKILL` and one wait, and reports what those two calls answered, alongside how long it waited, that budget, the descriptor ceiling the helper was closing against, and how the wait ended: on the helper's own report of the setup step that refused and the error it left, on the acknowledgement pipe closing with no report, or on the budget elapsing with nothing on the pipe. A helper that cannot finish its setup writes that report on the acknowledgement pipe it already owns before it ends, and the wait ends the moment the helper ends on every supported platform. On macOS the wait is a `select`, because `poll` on the FIFO the channel is built from never reports the writer's close. The point of reporting these is one distinction: a helper that had **already ended itself** before the signal, whose report or exit status names which of its own setup steps refused, against one that was **still running** and had to be killed, which says it was still working when the budget ran out. Nothing else is claimed, and in particular the message never says which process a number named.

**Which process the end of a helper names.** A pid is not evidence of the process holding it. An embedding host may reap this process's children from its own `SIGCHLD` handler with a wildcard wait; a helper collected there leaves its number free for the kernel to hand to another of that host's forks, and no observation the parent can make tells the two apart. Where the platform offers a name for a process that a reused number cannot impersonate, upstroke can take one in the parent the instant `fork` returns and end the helper through that name rather than through the number: on Linux a pid file descriptor, signalled with `pidfd_send_signal` and collected with `waitid` on the descriptor, neither of which can reach a process the descriptor does not name.

**That path is off unless the embedder turns it on, and `UPSTROKE_HELPER_IDENTITY=1` is how.** With it off — which is every deployment that has not set it — upstroke makes no `pidfd_open`, `pidfd_send_signal` or `waitid(P_PIDFD, ...)` call at all, and the end of a helper is the `kill` and the `waitpid` on its number that it has always been. **The trust boundary here is the host's syscall policy beside its platform.** A policy may *kill* the caller of a system call rather than refuse it — `SECCOMP_RET_KILL_PROCESS` is the disposition a disallowed call draws by default under systemd's `SystemCallFilter=` — and a process ended by a system call takes no fallback, so no answer upstroke can read establishes that making one of these calls is survivable, and no earlier answer stays true, because a filter may be installed at any point in a process's life. Setting the variable is the embedder asserting that its own host permits these three calls; upstroke makes none of them on any host where that assertion has not been made, and nothing it learned from one launch is carried into the next.

**Ending a helper by its number is best effort**, and that is where every deployment stands by default: against a host that reaps this process's children while a helper is starting or ending, the signal and the wait may reach a process that is not the helper. Upstroke asks no obligation of an embedding host for it; it states which platforms and policies it can answer for and which it cannot.

**With the path on, what a call answers is answered where that call is made.** A policy that refuses one of the three with an errno leaves this host where a host with no such name stands, and the number is what the refused call falls back to — decided by the teardown's own call, because a filter selects on arguments and a launch's are not a teardown's. The two errnos that would mean the helper has ended and been collected, `ESRCH` from the signal and `ECHILD` from the wait, are acted on only where a second call through the same descriptor agrees that the process it names is gone: a descriptor names one process and never another, so both are answers about the helper, and where they disagree nothing has been established and the number is what is left. A policy that answers *both* calls falsely is not distinguishable from a kernel, and neither is an `ESRCH` written for `pidfd_open` itself, which has no descriptor yet to ask a second question through; both are inside what turning the path on asserts. **What never falls back is a name this platform offers and this launch could not take**: a descriptor the process had run out of says nothing about the facility, so the launch fails and the helper ends on its closed command pipe rather than under a signal aimed at a number — and it is waited for by number there, within the same startup budget the launch gives a helper, because a wait reaches none but this process's own children and an uncollected child's number is not re-issued, where a `kill` aimed at that number has no such guarantee. The wait is bounded rather than blocking because a helper still inside its own startup need not end when its pipe closes, and what the wait answered goes into the failure. Beyond that name and the pipe it was already reading, the parent asks the kernel nothing about the helper, and the message says only what the pipe carried and what the signal and the wait answered.
