...
**Which process the end of a helper names.** A pid is not evidence of the process holding it. An embedding host may reap this process's children from its own `SIGCHLD` handler with a wildcard wait; a helper collected there leaves its number free for the kernel to hand to another of that host's forks, and no observation the parent can make tells the two apart. So where the platform offers a name for a process that a reused number cannot impersonate, the launch takes one in the parent the instant `fork` returns and ends the helper through that name rather than through the number: on Linux a pid file descriptor, signalled with `pidfd_send_signal` and collected with `waitid` on the descriptor, neither of which can reach a process the descriptor does not name; and where the helper was collected before any name could be taken, the end reaches nothing at all rather than falling back to the number. Where no such name can be had — macOS, any supported Unix whose kernel lacks the facility, and a policy that refuses any of the calls the name is used through — the end stays a `kill` and a `waitpid` on the number, and **ending a helper there is best effort**: against a host that reaps this process's children while a helper is starting or ending, it may reach a process that is not the helper. A refusal is established by making the call and not by predicting it, because a policy may select on a call's arguments and a launch's arguments are not a teardown's; so a signal or a wait through the name that the host refuses falls back to the number where it is refused, which is the same best effort and the same platform row. **What never falls back is a name this platform offers and this launch could not take**: a descriptor the process had run out of says nothing about the facility, so the launch fails and the helper ends on its closed command pipe rather than under a signal aimed at a number. Upstroke asks no obligation of an embedding host here; it states which platforms it can answer for and which it cannot. Beyond that name and the pipe it was already reading, the parent asks the kernel nothing about the helper, and the message says only what the pipe carried and what the signal and the wait answered.
