...
`546cee9b612e8fc6351cebfc257679d72e5be10f`: commit an initialised submodule at `reviews/findings`,
with a finding-shaped regular file at the submodule's root. The superproject records
`reviews/findings` as mode **`160000`**, and `git status --porcelain` returns exit 0 with empty
output — the checkout is clean.

- building the three tree listings: **exit 0**, and validating them: **exit 1, `names no finding`**
- passing the same checkout's `reviews/findings` **directory**: **exit 0, `conforms`**
...
access can open a pull request from a fork that commits a submodule at `reviews/findings`.
