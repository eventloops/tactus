...
**Not caused by the change in front of it.** PR #262's diff is confined to `reviews/findings/`; `git diff origin/master...HEAD -- src/ Cargo.toml Cargo.lock` is empty, so the `src/` tree the table above pins is `master`'s. Linux and macOS were green at the red head — on Linux `\\server\share\bin` is not absolute, so the fixture takes the skipped branch and never reaches a `stat` at all.
