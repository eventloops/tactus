...
Until both exist, the honest statement of what widening achieves is that it **records the wider reservation for future allocations** and does not protect against a worker already running in the module it adds. The handoff and PR #262's body were corrected to say exactly that on 2026-09-10; this row is the reason they had to be.

**Do not read this as a reason to keep an under-reserved row.** Widening still removes the module from the next allocation, which is where most of the exposure is. It is the *retroactive* protection the instruction implied that does not exist.
