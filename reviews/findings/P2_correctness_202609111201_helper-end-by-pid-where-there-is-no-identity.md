...
On macOS, on any supported Unix whose kernel has no `pidfd_open`, and on any host whose policy refuses one of the three calls an identity is used through -- `pidfd_open`, `waitid(P_PIDFD, ...)` or `pidfd_send_signal` -- `open_helper_identity` answers `-1` and the end of a private helper is `kill(pid, SIGKILL)` and `waitpid(pid, ..., 0)` on the number -> an embedding host whose `SIGCHLD` handler reaps this process's children with a wildcard wait collects a helper that died before READY, and the kernel re-issues its number to another of that host's forks -> the `SIGKILL` kills that process and the `waitpid` collects it, taking its exit status from the host and blocking the launch for as long as it runs. This is the closed row's sequence unchanged; what changed is that it is now confined to the platforms with no identity, and that DESIGN §15 states it there rather than leaving it implied.
