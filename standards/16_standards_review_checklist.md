...
- [ ] Lint levels change only in `[lints]`, except §2's two carve-outs: a module-level `#![deny]`
      activating a lint `[lints]` does not yet govern, and the allowlisted attributes that carry
      the `clippy.toml` `disallowed-*` census. A new suppression is a narrow, reasoned `#[expect]`
      on a leg that compiles the region (§2, §11) — below module level for a censused lint only
      where an `effects/allowlist.toml` row records that lint and counts the sites — or the
      module-level `allow` of a governed lint that such a row records, which is `allow` rather than
      `expect` because the row's lint set is the union over platforms (that file's header says why).
