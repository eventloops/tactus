...
The lineage clause is defence in depth rather than a live path. No legal
log reaches the state it refuses — a task holding an open generation
while some member of its lineage holds an open question — because the
fold's own checks refuse both orders that would build it.
`check_dispatched` refuses the dispatch-after-question order on
`lineage_has_question`, this same predicate, with "dispatch requires no
