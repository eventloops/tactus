...
terminated — and [`UpstrokeError::Filesystem`] from half (b) when the runs
directory exists and cannot be enumerated. A reclaim or a repair that fails in
half (b) is a [`RunDirOutcome::Unreclaimable`] entry and not an error.
...
[`UpstrokeError::Filesystem`] when `<repo>/.upstroke/runs` exists and cannot be
read, or an entry in it cannot be. A census that reported success having
scanned nothing would convert INV-15's "reclaims pre-run husks at
write-command start" from an unproven claim into an apparently-proven one.
Phase 1 is otherwise read-only and cannot fail.
...
The ID view skips names that are not UTF-8; worktree acquisition has already
checked cleanup leases using every exact filesystem name. Only proven
runs-root absence is empty. Open, entry, and metadata failures propagate
before the census can apply any plan.
