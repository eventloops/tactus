...
refusing to preview a plan over one would be a strange trade. It is not
silence either: a run-directory enumeration that did not answer is reported
as a warning saying the capacity block below rests on rate-limit signals
alone, which is the one thing "no runs" would have said falsely.
