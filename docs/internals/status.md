...
What `status` says about a husk id, `Ok(None)` if `wanted` names no husk, and
the enumeration's own failure when the husk listing did not answer.
...
An enumeration that failed is not a repository without husks, so it is not
folded into `Ok(None)`: `list_husks` and `RepoKey::for_repo` both propagate,
and the operator is told what could not be read rather than being told that
...
Only a refusal is a candidate for that second look. Any other error from
`resolve_run_id` is an observation that did not happen, and it is returned as
itself rather than being replaced by an answer about husks.
