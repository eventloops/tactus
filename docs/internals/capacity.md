...
Three arms, not two. A repository with no runs and a repository whose runs
could not be enumerated both leave the observations empty, but only the
second is a failure, and it is reported as a warning beside the signals
rather than passed off as an absence of spend.
